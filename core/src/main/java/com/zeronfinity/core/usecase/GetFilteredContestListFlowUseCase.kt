...
class GetFilteredContestListFlowUseCase(
    ...
    operator fun invoke(): Flow<List<Contest>> =
        contestRepository.getContestListFlow().map { list ->
            list.filter { contest ->

                var startTimeLowerBound =
                    filterTimeRepository.getFilterTimeRange(START_TIME_LOWER_BOUND)
                var startTimeUpperBound =
                    filterTimeRepository.getFilterTimeRange(START_TIME_UPPER_BOUND)
                var endTimeLowerBound =
                    filterTimeRepository.getFilterTimeRange(END_TIME_LOWER_BOUND)
                var endTimeUpperBound =
                    filterTimeRepository.getFilterTimeRange(END_TIME_UPPER_BOUND)
                val durationLowerBound =
                    filterTimeRepository.getFilterDuration(DURATION_LOWER_BOUND)
                val durationUpperBound =
                    filterTimeRepository.getFilterDuration(DURATION_UPPER_BOUND)
                ...
                if (filterTimeRepository.isLowerBoundToday(START_TIME)) {
                    ...
                    calendar.add(
                        ...
                        filterTimeRepository.getDaysAfterToday(START_TIME)
                    )
                ...
                if (filterTimeRepository.isLowerBoundToday(END_TIME)) {
                    ...
                    calendar.add(
                        Calendar.DAY_OF_YEAR,
                        filterTimeRepository.getDaysAfterToday(END_TIME)
                    )
